            ...
            return (
              <Fragment key={index}>
                <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-start">
                  <BsFillPersonFill className="mx-2 w-16 h-16 text-success rounded-full"/>
                  <div className="mx-2 flex justify-between w-full">
                    <div className="flex flex-col">
                      <div className="flex  flex-col">
                        <span className="fs-4 fw-bolder text-uppercase">{item.user ? item.user.name : ""}</span>
                        <span className="text-sm text-success fs-6">
                          {moment(item.createdAt).format("l")}
                        </span>
                      </div>
                      <div className="leading-tight mt-3 fw-bold text-capitalize ">{item.review}</div>
                    </div>
                    <div className="flex flex-col">
                      <div className="flex">
                        {/* Yellow Star */}
                        {[...Array(Number(item.rating))].map((index) => {
                          return (
                            <span key={index}>
                              <svg
                                className="w-4 h-4 fill-current text-danger"
                                fill="currentColor"
                                viewBox="0 0 20 20"
